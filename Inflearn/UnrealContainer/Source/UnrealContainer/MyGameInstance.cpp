...
void UMyGameInstance::Init()
{
	...
	Algo::Transform(StudentData, AllStudentsNames, [](const FStudentData& Value)
	{
		return Value.Name;
	});
	...
	Algo::Transform(StudentData, AllUniqueNames, [](const FStudentData& Value)
	{
		return Value.Name;
	});

	UE_LOG(LogTemp, Log, TEXT("단일 학생 이름의 수 : %d"), AllUniqueNames.Num());
	...
	TMultiMap<FString, int32> StudentMapByMap;
	Algo::Transform(StudentData, StudentMapByMap, [](const FStudentData& Val)
	{
		return TPair<FString, int32>(Val.Name, Val.Order);
	});

	UE_LOG(LogTemp, Log, TEXT("중복 가능한 이름에 따른 학생 맵의 레코드 수 : %d"), StudentMapByMap.Num());

	const FString TargetName(TEXT("이혜은"));
	TArray<int32> AllOrders;
	StudentMapByMap.MultiFind(TargetName, AllOrders);
